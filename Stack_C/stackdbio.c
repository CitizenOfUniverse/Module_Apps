...
void load_stack_from_file(DBFile *db, stack* stack){
	int now_element;//Temporary varible for a new element
	while(fread(&now_element, sizeof(int), 1, db->file_ptr) == 1){//Cycle exit point is error of reading file or end of file
...
// Note: file contains a stack in reversed order
void save_stack_to_db(DBFile *db, stack* stack){
	while (stack->size > 0){
		node* now_node = stack->top;
		if (fwrite(&(now_node->data), sizeof(int), 1, db->file_ptr) != 1){
			error_handler_stack(ERR_WRITE_FILE, stack);
		}
